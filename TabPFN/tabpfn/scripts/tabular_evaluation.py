...
def eval_model(i, e, valid_datasets, test_datasets, eval_positions, bptt, add_name, base_path, device='cpu',
               eval_addition='', **kwargs):
    metrics_test, config_sample, model_path = eval_model_on_ds(i, e, test_datasets, eval_positions, bptt, add_name,
                                                               base_path, device=device, eval_addition=eval_addition,
                                                               **kwargs)
    metrics_valid, _, _ = eval_model_on_ds(i, e, valid_datasets, eval_positions, bptt, add_name, base_path,
                                           device=device, eval_addition=eval_addition, **kwargs)
    return {'mean_auc_test': metrics_test['mean_roc_at_1000'], 'mean_auc_valid': metrics_valid['mean_roc_at_1000'],
            'mean_ce_test': metrics_test['mean_ce_at_1000'], 'mean_ce_valid': metrics_valid['mean_ce_at_1000'],
            'config_sample': config_sample, 'model_path': model_path}


def eval_model_on_ds(i, e, valid_datasets, eval_positions, bptt, add_name, base_path, device='cpu', eval_addition='',
                     **kwargs):
    ...
    metrics_valid = evaluate(datasets=valid_datasets, model=model[2], method='transformer', device=device,
                             overwrite=True,
    ...
    tabular_metrics.calculate_score_per_method(tabular_metrics.auc_metric, 'roc', metrics_valid, valid_datasets,
                                               eval_positions)
    tabular_metrics.calculate_score_per_method(tabular_metrics.cross_entropy, 'ce', metrics_valid, valid_datasets,
                                               eval_positions)
    ...
    for [ds_name, X, y, categorical_feats, _, _] in datasets:
        ...
        for eval_position in (eval_positions if verbose else eval_positions):
            ...
            if not return_tensor:
                make_scalar = lambda x: float(x.detach().cpu().numpy()) if (
                        torch.is_tensor(x) and (len(x.shape) == 0)) else x
        ...
        if num > 0:
            aggregated_metric_datasets, num_datasets = (
                    aggregated_metric_datasets + (aggregated_metric / num)), num_datasets + 1
...
def evaluate_position(X, y, categorical_feats, model, bptt
                      ...
                      , per_step_normalization=False, load_only=False, **kwargs):
    """
    ...
    if save == "backup":
        if not Path(path + "-low-perf-backup").exists():
            shutil.copy(path, path + "-low-perf-backup")
