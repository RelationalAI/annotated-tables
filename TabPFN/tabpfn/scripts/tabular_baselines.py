...
def eval_f(params, clf_, x, y, metric_used):
    ...
    if (get_scoring_string(metric_used, usage='sklearn_cv') == 'r2' or
            get_scoring_string(metric_used, usage='sklearn_cv') == 'neg_log_loss'):
...
def transformer_metric(x, y, test_x, test_y, cat_features, metric_used, max_time=300, device='cpu',
                       N_ensemble_configurations=3, classifier=None):
...
def autogluon_metric(x, y, test_x, test_y, cat_features, metric_used, max_time=300, **kwargs):
    ...
    predictor = TabularPredictor(
        ...
        excluded_model_types=['GBM', 'CAT', 'NN'],
    ...
    return metric, pred, predictor.fit_summary(verbosity=0)
...
def well_tuned_simple_nets_metric(X_train, y_train, X_test, y_test, categorical_indicator, metric_used, max_time=300,
                                  nr_workers=1, **kwargs):
    """Install:
    ...
    with tempfile.TemporaryDirectory(prefix=f"{len(X_train)}_{len(X_test)}_{max_time}") as temp_dir:
        ...
        X_train = pd.DataFrame(
            {i: safe_int(X_train[:, i]) if c else X_train[:, i] for i, c in enumerate(categorical_indicator)})
        X_test = pd.DataFrame(
            {i: safe_int(X_test[:, i]) if c else X_test[:, i] for i, c in enumerate(categorical_indicator)})
...
## AUTO Sklearn
def autosklearn_metric(x, y, test_x, test_y, cat_features, metric_used, max_time=300, **kwargs):
    import autosklearn.classification
    return autosklearn2_metric(x, y, test_x, test_y, cat_features, metric_used, max_time=max_time, version=1)


def autosklearn2_metric(x, y, test_x, test_y, cat_features, metric_used, max_time=300, version=2, **kwargs):
...
def lightautoml_metric(x, y, test_x, test_y, cat_features, metric_used, max_time=300, **kwargs):
...
def lightgbm_metric(x, y, test_x, test_y, cat_features, metric_used, max_time=300, no_tune=None, **kwargs):
    x, y, test_x, test_y = preprocess_impute(x, y, test_x, test_y
                                             , one_hot=False, impute=False, standardize=False
                                             , cat_features=cat_features)
    ...
    def clf_(**params):
        return LGBMClassifier(categorical_feature=cat_features, use_missing=True
                              , objective=get_scoring_string(metric_used, usage='lightgbm',
                                                             multiclass=len(np.unique(y)) > 2),
                              num_classes=len(np.unique(y)), **params)
...
def logistic_metric(x, y, test_x, test_y, cat_features, metric_used, max_time=300, no_tune=None, device='cpu'):
    x, y, test_x, test_y = preprocess_impute(x, y, test_x, test_y
                                             , one_hot=True, impute=True, standardize=True
                                             , cat_features=cat_features)
...
def gradient_boosting_metric(x, y, test_x, test_y, cat_features, metric_used, max_time=300, no_tune=None, **kwargs):
...
## SVM
param_grid_hyperopt['svm'] = {'C': hp.choice('C', [0.1, 1, 10, 100]), 'gamma': hp.choice('gamma', ['auto', 'scale']),
                              'kernel': hp.choice('kernel', ['rbf', 'poly', 'sigmoid'])}


def svm_metric(x, y, test_x, test_y, cat_features, metric_used, max_time=300, no_tune=None, **kwargs):
    x, y, test_x, test_y = preprocess_impute(x, y, test_x, test_y,
                                             one_hot=True, impute=True, standardize=True,
                                             cat_features=cat_features)
...
def gp_metric(x, y, test_x, test_y, cat_features, metric_used, max_time=300, no_tune=None, **kwargs):
    x, y, test_x, test_y = preprocess_impute(x, y, test_x, test_y,
                                             one_hot=True, impute=True, standardize=True,
                                             cat_features=cat_features)
...
def tabnet_metric(x, y, test_x, test_y, cat_features, metric_used, max_time=300, **kwargs):
...
def xgb_metric(x, y, test_x, test_y, cat_features, metric_used, max_time=300, no_tune=None, gpu_id=None, device='cpu'):
    import xgboost as xgb
    # XGB Documentation:
...
def ridge_metric(x, y, test_x, test_y, cat_features, metric_used, **kwargs):
    import warnings
    def warn(*args, **kwargs):
        pass
